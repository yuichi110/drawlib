...
def list_dependencies(dependencies: list[DependencyInfo]):
    """Print a cleaned list of dependency package names."""
    for dep in dependencies:
        print(dep.name)


def get_releases(package_name: str, year_from: int | None, year_to: int | None) -> list[dict]:
...
def list_releases(releases: list[ReleaseInfo], format: str = "text"):
    if format == "json":
        print(
            json.dumps(
